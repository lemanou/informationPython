...
def sentPlotting(sentList):
    ...
    plt.bar(center, hist, align='center', width=width, color='blue', alpha=0.5, label='Body')
    plt.bar(center, hist2, align='center', width=width, color='red', alpha=0.5, label='Comments')
...
def startPlotting(dbConn):
    ...
    for article in dbConn:
        if (len(article) is 7): # 7, 16, 18
            myArticle = dataToCouchDB.loadArticleFromDB(dbConn, article)
            sentList.append(myArticle['sentiment'])

    print "===Number of normal articles found: " + str(len(sentList))
    sentPlotting(sentList)
